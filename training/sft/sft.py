...
from training.utils import is_wandb_logged_in
import torch
...
def preprocess(
    ...
    max_len: int,
    system_message: str = Prompts.SYSTEM_PROMPT_CREDIT_SCORING.value
) -> Dict:
...
def align_data_format(data: Dict) -> Dict:
    """Transform the cot reasoning steps data to the format comes with Qwen official script."""
    # use only correct classifications
    data_correct = [
        item for item in data if item["pred_label"] == item["gold_label"]
        ]
    aligned_data = []
...
def make_supervised_data_module(
    ...
    train_json = align_data_format(train_json)
...
def train():
    ...
    if wandb_args.use_wandb:
        ...
        training_args.logging_dir="logs/sft"
