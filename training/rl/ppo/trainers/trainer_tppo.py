...
class TPPOTrainer:
    ...
    def ppo_update(self, sample):
        ...
        for start in range(0, batch_size, cp_batch_size):
            ...
            pi_logits = pi_logits.view(obs_batch[start:end].shape[0], -1, *pi_logits.shape[-2:])
            ...
            policy_loss, approx_kl, entropy_value = self.cal_policy_loss(log_prob_infer, log_prob_batch[start:end], advantages_batch[start:end], 
                                                          entropy, token_mask[start:end])
    ...
    def train(self, buffer):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                value_loss, value_grad_norm, policy_loss, policy_grad_norm, entropy = self.ppo_update(sample)
