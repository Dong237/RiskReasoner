...
def separate_steps(steps: List[str], mode: str = 'join') -> Any:
    """
    Helper function to join or split reasoning steps.

    Depending on the mode, this function either joins a list of steps into a 
    single string with a delimiter or splits a string into individual steps.

    Parameters:
        steps (List[str] or str): A list of reasoning steps (for 'join' mode) 
            or a single string containing concatenated steps (for 'split' mode).
        mode (str): The operation mode, either 'join' to concatenate steps 
            or 'split' to separate them.

    Returns:
        Any: A single string (if mode is 'join') or a list of strings (if mode is 'split').

    Raises:
        TypeError: If `steps` is not a list for 'join' mode or not a string for 'split' mode.
...
def check_correctness(generated_response: str, expected_answer: str) -> bool:
    """
    Check if the generated response contains the expected answer.

    This function splits the generated response into sentences and checks if the
    expected answer is present in the last sentence.

    Parameters:
        generated_response (str): The response generated by the language model.
        expected_answer (str): The correct answer to validate against.

    Returns:
        bool: True if the expected answer is found in the last sentence of the response, 
            False otherwise.
    """
    ## Original splitting method from openr, not working well in risk reasoner
    # sentences = re.split(
    #     r'(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?)\s', generated_response.strip()
    # )
    # last_sentence = sentences[-1] if sentences else ''
    last_sentence = generated_response.lower().split(SPLIT_TOKEN.lower())[-1]
    return expected_answer.strip() in last_sentence.strip() # make sure to use lower case
...
class SearchTree:
    def __init__(self):
        ...
        self.nodes: List[State] = []  # List of all states including the root
...
class OmegaPRM:
    def __init__(self, LM: LanguageModel,  c_puct: float, alpha: float, beta: float, L: int, k: int, N: int,
                 rollout_budget: int, save_data_type: str):
        """
        ...
        - k (int): Number of rollouts to generate in total (the denominator) for Monte Carlo estimation.
        ...
        self.n = 0  # counter for counting the search iterations
    ...
    def run(self, question: str, answer: str) -> Tuple[List[dict], List[dict]]:
        """
        ...
        if self.save_data_type == 'tree':
            data_tree = self.collect_tree_structure()
            return data_tree, None
        elif self.save_data_type == 'text':
            data_text = self.collect_solution_prefixes()
            return None, data_text
        elif self.save_data_type == 'both':
            data_tree = self.collect_tree_structure()
            data_text = self.collect_solution_prefixes()
            return data_tree, data_text
        else:
            raise ValueError("Invalid save_data_type. Please choose 'tree', 'text', or 'both'.")
    ...
    def monte_carlo_estimation(self, state: State):
        ...
        if state.MC == 1.0:
            ...
            for rollout in incorrect_rollouts:
                # calcualte the priority / tree statistics and add them to a max heap
    ...
    def compute_Q(self, state: State, rollout: str) -> float:
        ...
        word_count = len(rollout.split())  # TODO: When it comes to Chinese, this has to be changed.
    ...
    def compute_U(self, state: State) -> float:
        ...
        """
        N_total = sum(s.N for s in self.T.nodes) # N_total 
        if N_total == 0:
